...
main(void)
{
    ...
    for (x = 0; x < BUF_SIZE; x++)
        total = total + readCircBuf (&g_inBuffer);
    // Calculate and display the rounded mean of the buffer contents
    ground_height =  (2 * total + BUF_SIZE) / 2 / BUF_SIZE;
    //ground_height = 2000;
    ...
    while (1)
    {
        ...
        displayMeanVal (ground_height, g_ulSampCnt);
        check = check + 1;
        if (check == 4) {

            ground_height = ((2 * sum + BUF_SIZE) / 2 / BUF_SIZE);
        }
